...
template<typename dist_t, bool enable_buffer, bool unlink_same_set>
class SetHNSW {
public:
    class VisitedList {
    public:
    ...
    SetGraphIndex<enable_buffer, unlink_same_set>* outer_index;

    SetHNSW(hnswlib::SpaceInterface<dist_t>* space, size_t max_elements, size_t M = 16, size_t ef_construction = 200,
        ...
        while (!candidate_set.empty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (is_search) {
                    ...
                    if (enable_buffer) {
                        sid_t sid = outer_index->v2s[nei_id];
                        lid_t lid = outer_index->v2l[nei_id];
                        int offset = outer_index->set_off[sid] + lid;
                        outer_index->dist_buffer[offset] = dist;
                    }
                }
