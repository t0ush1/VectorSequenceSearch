...
public:
    struct Status {
        ...
        bool operator<(const Status& other) const {
            return q_sidx > other.q_sidx || (q_sidx == other.q_sidx && dist < other.dist);
        }
    ...
    std::unordered_map<label_t, id_t> label_to_id;
    std::vector<slabel_t> id_to_slabel;
    ...
    SequenceHNSW(hnswlib::SpaceInterface<dist_t>* space, size_t max_elements, size_t M = 16,
        ...
        this->id_to_slabel.resize(max_elements);
    ...
    void add_point(const void* point, label_t label, slabel_t slabel, int sindex) {
        id_t cur_id = cur_elements++;
        int cur_level = get_random_level();
        element_levels[cur_id] = cur_level;

        label_to_id[label] = cur_id;
        id_to_slabel[cur_id] = slabel;
        id_to_sindex[cur_id] = sindex;

        memset(addr_element(cur_id), 0, size_element);
        memcpy(addr_data(cur_id), point, data_size);
        ...
        if (cur_level < max_level) {
            ep_id = search_down_to_level<false>(enterpoint, point, cur_level);
        }

        for (int level = std::min(cur_level, max_level); level >= 0; level--) {
            auto top_candidates = search_level<false>(ep_id, point, level);
    ...
    // TODO 是否需要去重，检查一下重边有几条
    void add_successors(const std::vector<label_t>& sequence) {
        int seq_len = sequence.size();
        for (int i = 0; i < seq_len; i++) {
            linklist_t* ll = addr_link_level0(label_to_id[sequence[i]]);
            int size = get_ll_size(ll);
            id_t* neighbors = get_ll_neighbors(ll);
            for (int j = 1; j <= suc_M0 && i + j < seq_len; j++) {
                neighbors[size++] = label_to_id[sequence[i + j]];
            }
            set_ll_size(ll, size);
        }
    }
    ...
    std::priority_queue<Status> search_level_dp(id_t ep_id, const void* query_seq, int query_len, int level) {
        ...
        while (!candidate_set.empty()) {
            ...
            linklist_t* ll = addr_linklist(st.id, level);
            int size = get_ll_size(ll);
            id_t* neighbors = get_ll_neighbors(ll);
    ...
    std::unordered_set<slabel_t> search_candidates(const void* query_seq, int query_len) {
        id_t ep_id = search_down_to_level<true>(enterpoint, query_seq, 0);
        auto top_candidates = search_level_dp(ep_id, query_seq, query_len, 0);

        std::unordered_set<slabel_t> unique;
        while (!top_candidates.empty()) {
            unique.insert(id_to_slabel[top_candidates.top().id]);
            top_candidates.pop();
        }
        return unique;
    }
